...
function createEmptyDefinition() {
    return { version: SCHEMA_VERSION, variables: [] };
}
...
function normalizeVariable(raw, index) {
    ...
    const variableType = VARIABLE_TYPES.has(variable.variableType)
        ? variable.variableType
        : (promptToggleMode ? 'dropdown' : legacyType);
    const promptType = ['dropdown', 'single', 'multi'].includes(variable.promptType)
        ? variable.promptType
        : (promptToggleMode && ['dropdown', 'single', 'multi'].includes(legacyType) ? legacyType : 'dropdown');
    const type = promptToggleMode ? promptType : variableType;
    const options = Array.isArray(variable.options)
        ? variable.options.slice(0, 100).map(normalizeOption)
        : [];
    const normalizedPromptOptions = Array.isArray(variable.promptOptions)
        ? variable.promptOptions.slice(0, 200).map(normalizePromptOption).filter(option => option.promptIdentifier)
        : [];
...
function normalizeDefinition(raw) {
    ...
    const variables = Array.isArray(source.variables)
        ? source.variables.slice(0, 50).map(normalizeVariable)
        : [];
...
function refreshSetvarBindings() {
    ...
    const protectedVariables = currentDefinition.variables.filter(variable =>
        !variable.promptToggleMode && variable.setvarMode);
    ...
    for (const variable of protectedVariables) {
        ...
        try {
            originalValue = descriptor
                ? ('value' in descriptor ? descriptor.value : descriptor.get?.call(store))
                : undefined;
        } catch {
        ...
        Object.defineProperty(store, variable.key, {
            ...
            enumerable: false,
            get() {
                const activeVariable = currentDefinition.variables.find(item =>
                    item.setvarMode && !item.promptToggleMode && item.key === variable.key);
...
function getNativePromptOrder(settings) {
    ...
    return orderGroups.find(group => String(group?.character_id) === '100001')?.order
        ?? orderGroups[0]?.order
        ?? [];
}
...
function getToggleableNativePromptCatalog() {
    ...
    const renderedToggleIds = new Set(
        [...document.querySelectorAll('#completion_prompt_manager_list [data-pm-identifier] .prompt-manager-toggle-action')]
            .map(toggle => toggle.closest('[data-pm-identifier]')?.dataset.pmIdentifier)
            .filter(Boolean),
    );
    return renderedToggleIds.size > 0
        ? catalog.filter(prompt => renderedToggleIds.has(prompt.identifier))
        : catalog;
}
...
function refreshOpenPromptInspector() {
    ...
    const row = [...document.querySelectorAll('#completion_prompt_manager_list [data-pm-identifier]')]
        .find(item => item.dataset.pmIdentifier === inspectedPromptIdentifier);
...
function reflectNativePromptState(identifier, enabled) {
    const row = [...document.querySelectorAll('#completion_prompt_manager_list [data-pm-identifier]')]
        .find(item => item.dataset.pmIdentifier === identifier);
    if (!row) return;
...
async function importDefinitionFile(file) {
    ...
    try {
        ...
        notify('success', result.warnings.length
            ? `가져오기 완료 · ${result.warnings.length}개 경고 확인 필요`
            : '변수 정의를 검증하고 가져왔습니다.');
    } catch (error) {
...
function isMacroNameAvailable(candidate, variableId) {
    const duplicate = currentDefinition.variables.some(variable =>
        variable.id !== variableId && variable.key.toLowerCase() === candidate.toLowerCase());
...
function renderSelectionEditor(variable, body) {
    const section = createEditorSection(
        'fa-list-check',
        '선택지',
        `${variable.options.length}개의 값을 구성하고 기본 선택을 지정합니다.`,
    );
    if (variable.type === 'multi') {
    ...
    variable.options.forEach((option, optionIndex) => {
        ...
        defaultInput.checked = variable.type === 'multi'
            ? variable.defaultValue.includes(option.id)
        ...
        const remove = createActionButton('삭제', () => {
            ...
            renderRuntimePanel();
            persistDefinition();
        }, 'prompt-controls-action-danger prompt-controls-option-remove', 'fa-trash-can');
    ...
    const addOption = createActionButton('선택지 추가', () => {
        ...
        renderRuntimePanel();
        persistDefinition();
    }, 'prompt-controls-action-primary prompt-controls-add-option', 'fa-plus');
...
function renderPromptToggleEditor(variable, body) {
    ...
    for (const option of variable.promptOptions) {
        ...
        defaultInput.checked = variable.type === 'multi'
            ? variable.promptDefaultValue.includes(option.id)
        ...
        const remove = createActionButton('등록 제거', () => {
    ...
    const closePicker = createActionButton('닫기', () => {
        picker.hidden = true;
    }, 'prompt-controls-prompt-picker-close', 'fa-xmark');
    ...
    const usedByOtherVariables = new Set(currentDefinition.variables
        .filter(item => item.id !== variable.id && item.promptToggleMode)
        .flatMap(item => item.promptOptions.map(option => option.promptIdentifier)));
    const available = getToggleableNativePromptCatalog()
    ...
    const addSelected = createActionButton('선택한 토글 추가', () => {
        const additions = available
            .filter(prompt => selected.has(prompt.identifier))
            .map(prompt => ({ id: makeId('prompt-option'), promptIdentifier: prompt.identifier, label: prompt.name }));
        ...
        queueNativePromptSync([variable]);
        persistDefinition();
    }, 'prompt-controls-action-primary', 'fa-plus');
    ...
    const addToggle = createActionButton('토글 추가', () => {
        picker.hidden = false;
        search.focus();
    }, 'prompt-controls-action-primary prompt-controls-open-prompt-picker', 'fa-plus');
...
function renderVariableCard(variable) {
    ...
    const basics = createEditorSection(
        'fa-sliders',
        '기본 설정',
        variable.promptToggleMode
            ? '표시 이름과 프롬프트 선택 방식을 관리합니다.'
            : '표시 이름, 변수 이름과 입력 방식을 관리합니다.',
    );
    ...
    const availableTypes = variable.promptToggleMode
    ...
    const copy = createActionButton('변수명', async () => {
        try {
    ...
    }, 'prompt-controls-action-primary', 'fa-copy');
    const remove = createActionButton('삭제', () => {
        ...
        updateRuntimeButton();
        persistDefinition();
    }, 'prompt-controls-action-danger', 'fa-trash-can');
...
function renderRuntimeSelection(variable, field) {
    const raw = getRawValue(variable);
    const options = getVariableOptions(variable);
    const promptCatalog = variable.promptToggleMode
        ? new Map(getNativePromptCatalog().map(prompt => [prompt.identifier, prompt]))
        : null;
...
function updateRuntimeButton() {
    ...
    runtimeButton.title = count > 0
        ? `User Preset Custom · ${count}개 변수`
        : 'User Preset Custom · 설정된 변수 없음';
}
